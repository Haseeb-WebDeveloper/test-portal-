...
// GET /api/client/contracts - Get contracts for clients with pagination and filtering
export async function GET(request: NextRequest) {
  try {
    ...
    const canViewContracts = userRecord.role === 'CLIENT' || 
    ...
    // Build where clause for contracts visibility
    const where: any = {
      deletedAt: null,
    };

    // Filter by client - if user is a client, show their contracts
    if (userRecord.role === 'CLIENT') {
      ...
      // If user is a client member, show contracts for their client
      const clientIds = userRecord.clientMemberships.map(membership => membership.clientId);
      where.clientId = { in: clientIds };
    }
